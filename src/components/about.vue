      ...
      <v-expand-x-transition>
        <v-card max-width="500" v-show="contributors" raised>
          <v-layout align-center justify-space-around column fill-height>
            ...
            <v-list dense>
              <v-list-tile v-for="(contributor, index) of getContributors()" :key="index">
                <v-list-tile-content class="align-end">{{ contributor.name }}</v-list-tile-content>
                <v-list-tile-action>
                  <a :href="`mailto:${contributor.email}`" v-if="contributor.email">
